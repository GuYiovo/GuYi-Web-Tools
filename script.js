...
// --- Web Worker 构建 (解决 JS 混淆卡死主线程问题) ---
// 我们动态创建一个 Blob URL 作为 Worker，这样不需要额外的 worker.js 文件
const workerCode = `
...
const appTemplate = `
    ...
    <main class="w-full max-w-6xl animate-fade-in relative z-10 p-4 md:p-8">
        
        <!-- 顶部导航栏 -->
        <nav class="absolute top-4 right-4 md:-top-8 md:right-0 flex gap-4 z-20">
            <button onclick="toggleAbout()" class="flex items-center gap-2 text-slate-400 hover:text-white transition-colors bg-black/20 hover:bg-black/40 backdrop-blur-md px-4 py-2 rounded-full text-sm border border-white/5 hover:border-white/20">
                <i class="ph-bold ph-info"></i>
                <span class="hidden md:inline">关于我们</span>
            </button>
        </nav>

        <header class="mb-8 text-center flex flex-col items-center mt-12 md:mt-0">
